...
import { AgroConsultantChat } from "@/components/support/AgroConsultantChat";
...
export default function Support() {
  return (
    <div className="space-y-0 pb-16">
      {/* Hero */}
      <section className="bg-gradient-to-b from-slate-500/10 to-background py-14">
        <div className="container mx-auto px-4 text-center space-y-4">
          <p className="text-sm uppercase tracking-[0.4em] text-muted-foreground font-medium">Поддержка</p>
          <h1 className="text-4xl md:text-5xl font-bold">
            Тех-Поддержка
          </h1>
          <p className="text-muted-foreground text-lg max-w-2xl mx-auto">
            Получите помощь от AI-агроконсультанта, живого специалиста или найдите ответ в базе знаний.
          </p>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            <Dialog>
              <DialogTrigger asChild>
                <Card className="hover:shadow-lg transition-shadow cursor-pointer h-full border-emerald-200 dark:border-emerald-800">
                  <CardHeader>
                    <div className="bg-emerald-500/10 p-3 rounded-full w-12 h-12 flex items-center justify-center mb-4">
                      <Bot className="w-6 h-6 text-emerald-600" />
                    </div>
                    <CardTitle className="text-emerald-700 dark:text-emerald-400">🌱 AI-Агроконсультант</CardTitle>
                    <CardDescription>Диалоговый помощник по полю, поливу и доходу</CardDescription>
                  </CardHeader>
                  <CardContent>
                    <Button className="w-full bg-emerald-600 hover:bg-emerald-700">Начать консультацию</Button>
                  </CardContent>
                </Card>
              </DialogTrigger>
